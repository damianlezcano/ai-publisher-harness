...
## Estado actual (FRESH REAL APPIMAGE POST-CONVERSATION-UX CONSTRUIDO DESDE MAIN `2b122e5` — BUILD + VERIFICACIÓN TÉCNICA COMPLETA, FINDINGS 1-6 VALIDADOS, TECHNICALLY READY FOR HUMAN RE-ACCEPTANCE, M11 NO INICIADO, 2026-09-02)

- **FRESH REAL APPIMAGE CONSTRUIDO DESDE MAIN `2b122e5` (HEAD = checkpoint del merge `6886ba9`) = PASS (sesión FRESH, OpenCode/DeepSeek V4 Flash, orquestador).** El AppImage previo `832408b6…` era **STALE** (construido desde `5c17e13`, predata el merge `6886ba9`). Packaging canónico M10 `scripts/smoke-package appimage`. **EXIT=0 (smoke-package PASS)**. Artefacto:
  `app/src-tauri/target/release/bundle/appimage/EducAI_0.1.0_amd64.AppImage`, **180.931.064 bytes**, **SHA-256 `a227d1d1805a15570b71cd9fa9c1d6d09fd12787d6d392a4584cdddfbd809ee5`** (NUEVO, difiere del stale `832408b6…`), timestamp 2026-09-02 17:57:14 -0300, source commit `2b122e5` (main HEAD, working tree clean antes del build, sin cambios de producto sin commitear). Build via `scripts/smoke-package appimage` (fetch-sidecars → `cargo tauri build --bundles appimage` → fallback documentado a appimagetool tras el error esperado de linuxdeploy en Fedora 44). **Frontend embebido NUEVO de este build:** el binario `usr/bin/educai` del payload embebe `assets/index-BnZwruSz.js` + `assets/index-UHkEOOtE.css` (hash FE nuevo — el diff del merge `6886ba9` incluye cambios FE de Conversation Details/composer/logs).
- **SIDECAR PINS VERIFICADOS EN PAYLOAD (byte-idénticos al sidecar fetcheado en este build):** opencode **1.18.25** (payload sha `d91e0d33…` = binario extraído; el pin del manifest `58a3729a…` es del tarball de origen, consistente con checkpoints previos) y cloudflared **2026.8.3** (payload sha `f29324fe…` = pin `config/components.json`). `--version` en payload: opencode `1.18.25`, cloudflared `2026.8.3`. Sin repin/upgrade silencioso.
- **`./scripts/verify` EXIT=0 EN MAIN POST-BUILD** (cargo fmt/clippy/test verdes, FE **228/228** en 21 archivos, format/lint/typecheck, fetch-sidecars --check, M10 version 0.1.0 + UX_REDESIGN_01 contracts, cargo check src-tauri, git diff --check). **Targeteds (todos en esta sesión, sin reuso de resultados viejos):** `opencode_adapter` **28/28**, `app_facade` **39/39**, `app_provider` **14/14**, `agent_service` **10/10**, `project-agent --lib` **16/16** (incl. `merge_artifacts_keeps_diff_and_does_not_scan_prior_files`), session_log unit tests (ring 500/clear/levels/process-local), preview `preview_security` **10/10** + `preview_lifecycle` **4/4** + `project-app --test preview` **9/9** (regresión-only, NO reabiertos).
- **LANZAMIENTO REAL FEDORA/WAYLAND (DISPLAY=:0) = PASS.** Procesos viejos EducAI (varias instancias del inode stale) y sidecars huérfanos de `.mount_EducAI*` TERMINADOS (no usados como evidencia). Lanzado el artefacto NUEVO (PID 821048, setsid, PATH restringido `/tmp/opencode/path-indep-bin:/usr/bin:/bin`, `--debug`, stderr capturado) con log: `[EducAI][INFO] startup version=0.1.0` + `[agent] backend starting → ready`, **SIN falso error de arranque**. WebKitNetworkProcess + WebKitWebProcess activos (UI usable). Sidecar hijo desde el mount propio del AppImage (`/tmp/.mount_EducAIeNAGFE/usr/bin/opencode`, port 42351, `/global/health` HTTP 200 `{"healthy":true,"version":"1.18.25"}`, `--version`=1.18.25; `readlink /proc/<pid>/exe` = mount del AppImage → **PATH-independencia PASS**, `command -v opencode/cloudflared` = fail en el PATH restringido). cloudflared **2026.8.3** en el payload del mount.
- **FINDING 6 = VALIDADO EN RUNTIME REAL (probe FRESH compilado contra main `2b122e5`, path-dep `crates/project-agent`, sidecar REAL pineado 1.18.25 — el MISMO binario bundlado en el AppImage, XDG aislado `/tmp/opencode/causality-runtime`).** **Fase A (turno fallido → switch de modelo → turno limpio):** T1 conversación `conversation-c` sesión `ses_f9c12b3c3ffeQvo0hWiKOdgfdo` modelo `missing/provider` → **T1 FAILED `TaskFailed("timed out")`**; T2 en sesión DISTINTA `ses_f9c1153ebffeEwOUqAYASay4yV` (**same_as_t1=false** — evicción de sesión cacheada tras fallo) modelo `opencode/big-pickle` → user `msg_063eeac1f001RBaCDFQN16rZc5` → assistant `msg_063eeac2a0012cbj8P8VFz9jBv` (parent=T2 user, `finish=stop`), **Completed** en 5.5s, mensaje SOLO el saludo `"Hola! 👋 ¿En qué puedo ayudarte hoy?"`, artifacts=[] → T1 NO resucitó, T1 NO se atribuyó a T2, sin eventos stale. **Fase B (turno cancelado → switch → turno limpio):** T3 `conversation-d` sesión `ses_f9c113e5dffeI0z38aBrCUBBoA` cancelado a los 3s → terminal `TaskFailed("timed out")` en el deadline acotado (90s, sin espera infinita), su assistant stale `msg_063eec1b2001lUhTRZro9f273U` (`finish=none`, parent=T3 user) NO se adjuntó; T4 en sesión DISTINTA `ses_f9c0fdea1ffeMLJh0zeSvls6jf` (**same_as_t3=false**) → `Completed` en 9.3s, mensaje SOLO `"¡Hola! ¿Cómo estás?"`, artifacts=[]. **Semántica validada en código (main):** ancla de turno INMUTABLE capturada tras `prompt_async` (opencode.rs:295-319, ids previos en `before_message_ids`), `message_belongs_to_turn` estricto `_ => false` (opencode.rs:502-510, sin fallbacks lenientes), evicción de sesión en TODOS los errores (`if result.is_err() { self.lock_sessions().remove(&session.project_id) }`, opencode.rs:351-353), fencing de workspace-scan a archivos ausentes al inicio del turno (service.rs:79-82, `merge_artifacts` fenced). Terminal states capturados: conversation ids, turn ids (session + message ids), model ids. Trace crudo: `/tmp/opencode/causality-fresh-trace.txt`.
- **FINDINGS 1-5 VALIDADOS (técnico, código + tests FE/Rust en esta sesión):** **(1)** `"Procesando tu solicitud…"` (`messages.ts:208` `agent.creating`) es transitorio: solo en `ChatPanel.tsx:241-246` bajo `agentPhase === "working"` como `.chat-status`, NUNCA persistido como mensaje assistant (la respuesta persistida es la real vía `append_assistant_message`); tests `ChatPanel.test.tsx:339,499` + `WorkspaceView.test.tsx:468`. **(2)** Conversation Details: el título es un botón (`WorkspaceView.tsx:215-222`, aria-label) que abre `ConversationDetails` (rename, modelo por-conversación vía `conversation_model_select`/`clear` — `ConversationDetails.tsx:44-45` —, Material subido con `materialOpenFolder`, Creaciones con `creationOpenFolder`); `key={project.id}` + `onClose` → cierra y vuelve a la MISMA conversación; sin control global competidor (ModelSelector eliminado; `api.modelSelect` no se invoca en la UI; Configuración = proveedores + Logs); aislamiento cubierto por `conversation_model_is_validated_persisted_isolated_and_clearable` (app_facade 39/39). **(3)** Lifecycle de adjuntos: `ComposerBar.send()` captura `ids` y hace `setPrompt("") + setAttachmentIds([]) + setShowMaterialPicker(false)` (ComposerBar.tsx:146-153) → el composer NO presenta chips/sugerencia stale tras send; el adjunto histórico queda representado en el material/timeline de la conversación (chips del user message en ChatPanel.tsx:223-237); test `"clears controlled attachment chips after send while materials remain available"` (ComposerBar.test.tsx:218). **(4)** Creation card: sin línea redundante "🎮 Actividad" (grep en CreationsPanel.tsx vacío); presentación concisa `kindLabel(creation.kind)` = `"Actividad interactiva"` (messages.ts:4, CreationsPanel.tsx:83) con icono aria-hidden + `sr-only` displayName; Abrir/Compartir preservados con el MISMO `creation.id` y aria-labels; test `CreationsPanel.test.tsx:40` (+ 12/12). **(5)** Logs de esta sesión: viewer en `ProviderPanel.tsx:105-131` ("Logs de esta sesión", Clear, Refresh, Copy vía `copyLogs`/clipboard); backend `session_log.rs` = ring in-memory de 500, `OnceLock+Mutex+VecDeque`, espejo stderr `[EducAI][LEVEL]`, niveles ERROR/WARN/INFO/DEBUG (default INFO), `configure_from_args` para `--debug`/`--log-level`; metadata-only (ids, chars, duraciones, model ids — NUNCA prompts/secrets/contenidos de attachments/HTML generado); unit tests `bounded_levels_and_clear_are_process_local` + `configure_from_args_sets_level_ordering`.
- **LOGGING EVIDENCE (traza de sesión actual REAL, probe FRESH `AppState` real + engine real vs sidecar 1.18.25, `/tmp/opencode/logging-fresh-trace.txt`):** `[EducAI][INFO] conversation created id=01a063f4-5079-7303-8619-fcd4e9c75c4a` → `turn accepted conversation_id=… message_id=01a063f4-507a-78f6-b308-6e3d5b505a63 chars=4` → `turn started conversation_id=… turn_id=01a063f4-507a-… model=opencode/big-pickle` → `[agent] backend starting/ready → session created → task started → task completed` → `turn terminal conversation_id=… turn_id=… status=completed creations=0 duration_ms=11404`. Ring final count=4 (solo eventos de ESTA sesión/proceso). **Arranque real del AppImage desde consola:** stderr capturó `[EducAI][INFO] startup version=0.1.0` (aplicación start) — el ring arranca VACÍO en un proceso nuevo (sin logs de sesiones previas tras restart, sin persistencia). Evidencia: `/tmp/opencode/educai-launch-fresh-2.log`. Sin payloads privados en el checkpoint.
- **REGRESIÓN-ONLY (NO reabierto, `./scripts/verify` EXIT=0 + targeteds):** "hola" contextual, chat secuencial, Creation request, Preview/Abrir (preview_lifecycle 4/4, preview_security 10/10, project-app preview 9/9), arquitectura de publicación real (publication suites verdes), Enter/Shift+Enter, badge compartido, delete «Sí», aislamiento de conversaciones — sin regresiones.
- **ESTADO DEL REPO:** main limpio (working tree clean antes y después del build), HEAD `2b122e5`, M11 NO INICIADO. Sin worktrees/branches/workers temporales (probes = crates temporales bajo `/tmp/opencode/causality-probe` y `/tmp/opencode/logging-probe`, fuera del repo). AppImage fresco en su ruta canónica.
- **MODELOS/POLÍTICA:** Cursor quota agotado — NO usado. GPT vía OpenCode Go PROHIBIDO en esta fase — NO usado. Fase 100% OpenCode/DeepSeek V4 Flash (orquestador), sin workers LLM (fase determinista: build + verify + probes contra sidecar real + tests). Session budget CONTINUE.
- **STATUS: TÉCNICAMENTE READY FOR HUMAN RE-ACCEPTANCE. NO HUMAN ACCEPTED.** Próximo gate y ÚNICO funcional: **HUMAN PRODUCT-OWNER RE-ACCEPTANCE** del AppImage fresco `a227d1d1…` (escenario: conversación nueva, "hola" → saludo real, pregunta normal, Creation con adjunto, secuencial, Preview, Compartir, detalles de conversación desde el título, cambiar modelo por conversación sin afectar otras, Logs de esta sesión, y el escenario Finding 6: modelo que falla → volver a modelo que funciona → "Hola" → respuesta SOLO de ese turno). **NO afirmar aceptación humana desde OpenCode. NO iniciar M11.**

## UNRESOLVED NEXT PACKAGING BLOCKER (registrado, NO abordado en esta sesión)

- **LINUX APPIMAGE PORTABILITY / REPRODUCIBLE BUILD BASELINE.** El AppImage construido en Fedora (`a227d1d1…`) fue observado fallando en KDE Neon 24.04 / Ubuntu Noble-family host porque las librerías bundladas requieren símbolos GLIBC más nuevos (GLIBC_2.42 / GLIBC_2.43 y requisitos ABI relacionados). **NO corregido en esta sesión por directiva.** Tras la re-acceptación funcional humana del AppImage fresco, se requiere un pass dedicado: **LINUX APPIMAGE PORTABILITY / REPRODUCIBLE BUILD BASELINE PASS** (build en un entorno baseline/glibc más viejo, verificación de símbolos GLIBC requeridos vs host destino, y definición de un baseline reproducible).
