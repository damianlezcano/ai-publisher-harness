...
## Estado actual (UX POLISH PASS — REVIEWS INDEPENDIENTES APPROVE, FIX ACOTADO INTEGRADO EN MAIN `ddc1bda`, VERIFICACIÓN VERDE, TECHNICALLY READY FOR HUMAN RE-ACCEPTANCE; M11 NO INICIADO; QUOTED-PROMPT Y GLIBC SIGUEN PENDIENTES, 2026-09-03)

- **PASS = COMPLETE.** Implementación UX polish (`169f2ea`, sesión orquestador previa) + **reviews independientes FRESH completados: 1 APPROVE + 1 REQUEST_CHANGES→fix acotado→re-review APPROVE, todo integrado en main**. El orquestador anterior rotó por ROTATE_SESSION_REQUIRED_HARD (~210K) antes de lanzar los reviewers; esta sesión FRESH los lanzó, aplicó el único fix acotado del Code/A11y review, re-reviewó FRESH, verificó e integró. **NO completado: HUMAN PRODUCT-OWNER RE-ACCEPTANCE** (el humano no probó aún tabs General/Logs, filas compactas ni delete responsivo).
- **UX1 — Configuración separado en tabs General/Logs (implementación `169f2ea`, preservada):** `ProviderPanel.tsx` separa **General** (nota de privacidad + proveedores Recomendados/Otros) y **Logs** ("Logs de esta sesión"). **General SIEMPRE es la pestaña activa al abrir** (`useState("general")` + remount de App.tsx `settingsOpen && <ProviderPanel/>`). **Logs NUNCA es default, NUNCA se recuerda entre aperturas ni tras reinicio** (estado local no persistido). Semántica ARIA tabs correcta (`role=tablist/tab/tabpanel`, `aria-selected`, `aria-controls`), roving tabindex + flechas ←/→, foco inicial en General vía `initialFocusRef`. Viewer de logs conserva TODO el comportamiento aceptado: buffer in-memory ring 500 solo-proceso (`OnceLock`+`Mutex`+`VecDeque`), Limpiar/Copiar/Actualizar, `pre.session-logs` acotado 220px con scroll interno, empty state "Sin eventos todavía.", anuncio aria-live, sin persistencia. Layout/navigation SOLO — backend de logging intacto.
- **UX2 — Listas de recursos compactas con fechas de dominio (`169f2ea`, preservada):** filas de 2 líneas: **primario** = nombre/título humano (materials `displayName`; creaciones `displayName` = título) 0.95rem; **secundario** = muted 0.8rem `humanSize · humanDate(createdAt)`; **Abrir** compacto. **Timestamps de dominio autoritativos (NO mtime, NO "now"):** materials = `Material.created_at` (registrado con `clock.now()` al importar, expuesto como `MaterialView.createdAt`); creaciones = `Creation.created_at` (idem `creation_view`). `humanDate` es-AR (`toLocaleString("es-AR", {dateStyle:"short", timeStyle:"short"})`), nunca timestamps crudos en UI. UNA "Abrir carpeta contenedora" por sección (2 total). Rows 61px en motor real WebKit; `.dialog-body` scrollea internamente. Typed previews/modelo por-conversación/rename intactos.
- **UX3 — Delete-confirmation responsivo (`169f2ea`, preservada):** `.dialog` base `max-width: min(420px, calc(100vw - 32px))` + media query `@media (max-width:520px)` (backdrop padding 8px, dialog padding 16px, `max-width: calc(100vw - 16px)`); `box-sizing: border-box` global; `.dialog-actions` `flex-wrap`. **Sin cambios de semántica de borrado:** afirmativo "Sí" (NFD + variantes), Eliminar disabled hasta válido, Cancelar nunca borra, Enter no bypasa, Escape cierra.
- **REVIEW PRODUCT/UX INDEPENDIENTE (OpenCode DeepSeek V4 Flash, FRESH, `ux-review`, worktree `../ai-publisher-ux-polish-review-ux` detached `169f2ea`) = APPROVE** (1/1, sin findings): General/Logs separados con General default y Logs explicit-only (`general_aria_selected=true` + `logs_panel_display:none` al abrir; tras click `display:block` con scroll interno acotado) verificados en report.json; Configuración responsiva 1200/800/520/420; Logs secundario pero descubrible; filas compactas 61px con 15 recursos + `.dialog-body` scroll interno; fechas de dominio es-AR sin "now" inventado; `folder_action_count=2` (una por sección); delete-confirmation responsivo (rects en viewport en 520/420, input completo border-box, Escape cierra, "Sí" habilita Eliminar); sin regresión de flujos aceptados. Único métrico no-cero (`doc_horizontal_overflow_px` en ≤520) = `min-width:800px` preexistente de `.app-shell`, fuera del scope.
- **REVIEW CODE/A11Y INDEPENDIENTE (OpenCode Qwen 3.8 Flash, FRESH, `code-review`, worktree `../ai-publisher-ux-polish-review-code` detached `169f2ea`) = REQUEST_CHANGES → FIX ACOTADO `ddc1bda` → RE-REVIEW FRESH (`code-review-2`, Qwen 3.8 Flash, worktree en `ddc1bda`) = APPROVE.** Finding 1 (bloqueante a11y): el focus-trap enumeraba TODOS los `FOCUSABLE_SELECTOR` incluyendo los botones del panel Logs `[hidden]` (Limpiar/Actualizar/Copiar) → el "último foco" calculado era un botón invisible y Tab tras el último control visible dejaba escapar el foco del modal. **Fix:** `useFocusTrap.ts` filtra el ciclo a nodos realmente enfocables (excluye descendientes de `[hidden]`, `disabled` y `aria-hidden=true`; deliberadamente NO `offsetParent` porque jsdom no tiene layout y rompería todos los tests de trap jsdom) + test nuevo `ProviderPanel.test.tsx` (foco en "Otros proveedores (1)" → Tab → wrap al botón Cerrar del header, nunca a botones ocultos ni a `document.body`). Finding 2 (minor): `className="confirm-dialog"` muerto (sin regla CSS) → eliminado de `ConfirmDialog.tsx` (el clamp base `.dialog` gobierna el ancho). Re-review FRESH Qwen = **APPROVE** (nits no bloqueantes: contador 241 del checkpoint previo stale por 1 línea — reescrito aquí; fallback de initial-focus sin filtro de visibilidad, teórico/out-of-scope porque todos los callers pasan `initialFocusRef` o tienen primer control visible).
- **VERIFICACIÓN FINAL EN MAIN `ddc1bda` POST-FIX: `./scripts/verify` EXIT=0** (cargo fmt/clippy/test verdes, FE **242/242** en 21 archivos — 241 previos + 1 test nuevo de focus trap —, format/lint/typecheck, fetch-sidecars --check, M10 0.1.0 + UX_REDESIGN_01 contracts, cargo check src-tauri, git diff --check). **Rust: 1142 tests pasados en 85 suites** (todo en verde). Tests FE que fijan el contrato: default-tab (General aria-selected + Logs hidden + foco en General), click explícito en Logs revela el viewer, flechas + modal no se cierra, reapertura siempre en General, contrato logs visible/bounded tras click, compact rows con `humanDate(createdAt)`, App.test settings (logs ocultos por default → click → visibles), focus trap no entra en el panel Logs oculto. `pnpm --dir app run build` previo OK (hashes `index-Bu6CW7uI.js` + `index-DIYuXvSr.css`); este fix NO cambia assets visuales (solo TS + clase muerta eliminada) → sin rebuild de dist necesario para la evidencia UI existente.
- **EVIDENCIA UI REAL (harness WebKitGTK 4.1, dist REAL, mock Tauri — `/tmp/opencode/fe-render/report.json` + 16 PNG):** Configuración diálogo en viewport (680/680/488/488 px) con General `aria-selected=true` + Logs `display:none` al abrir y `display:block` tras click con `session-logs` acotado; Detalles 15 filas @ 61px, `folder_action_count=2`, body scrollea; Delete diálogo 420/420/504/404 px, input completo border-box (368/368/468/368 px), "Sí" habilita Eliminar y Escape cierra en el motor real, sin overflow del diálogo. El fix de focus-trap NO altera geometría (no hay cambio CSS).
- **INTEGRACIÓN:** sin rama de feature que mergear — la implementación ya estaba commiteada en main (`169f2ea`); el doc de checkpoint (`642a5ce`) y el fix de review (`ddc1bda`) están encima. **HEAD main = `ddc1bda`, working tree clean.**
- **LIMPIEZA:** reviewers cerrados y panes Herdr limpios al cierre; worktrees de review `../ai-publisher-ux-polish-review-ux` y `../ai-publisher-ux-polish-review-code` retirados tras aprobarse; sin ramas temporales. Session budget de esta sesión: CONTINUE al inicio (~12K), CHECKPOINT_WARNING (~80K) al lanzar el re-reviewer, cerrado sin llegar a HARD STOP.
- **MODELOS/POLÍTICA:** Cursor NO usado. GPT vía OpenCode Go PROHIBIDO — NO usado (no se requirió Codex: el fix acotado fue apto para DeepSeek). Orquestador = OpenCode DeepSeek V4 Flash; Product/UX reviewer = OpenCode DeepSeek V4 Flash; Code/A11y reviewer = OpenCode Qwen 3.8 Flash (round 1 y re-review). **M11 NO INICIADO.**
- **PENDIENTES EXPLÍCITOS (NO tocados en este pass):** (1) **USER PROMPT QUOTING / SERIALIZATION** — "hola" funciona, `"hola"` falla; SIGUE SIENDO EL **PRÓXIMO PASS FUNCIONAL**. (2) **LINUX APPIMAGE GLIBC PORTABILITY** — Fedora-built AppImage falla en KDE Neon/Ubuntu 24.04 por símbolos GLIBC más nuevos; pass dedicado de packaging pendiente.
- **STATUS: TÉCNICAMENTE READY FOR HUMAN RE-ACCEPTANCE. NO HUMAN ACCEPTED.** Próximo gate y ÚNICO: **HUMAN PRODUCT-OWNER RE-ACCEPTANCE** del artifact UX polish (Configuración tabs General/Logs con General default y Logs explicit-only, filas compactas con fechas de dominio, delete-confirmation responsivo). El AppImage previo `b961080a…` es STALE tras `169f2ea`+`ddc1bda` → requiere rebuild fresco en el próximo gate. **NO afirmar aceptación humana desde OpenCode. NO iniciar M11.**
