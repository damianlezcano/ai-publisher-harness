...
## Estado actual (OPENCODE ASSISTANT MESSAGE SELECTION / FINAL RESPONSE SEMANTICS PASS — TRABAJO DE AUTOR COMPLETO Y VERIFICADO EN WORKTREE, ORQUESTADOR ROTADO ANTES DE REVIEWS/MERGE, M11 NO INICIADO, 2026-09-02)

- **PASS = TRABAJO DE AUTOR COMPLETO y VERIFICADO (4 commits en `corr/msg-selection-pass`, worktree `../ai-publisher-msg-selection-pass`, base main `405ecfe`), PERO NO REVISADO NI MERGEADO.** El orquestador (FRESH, deepseek-v4-flash) bootstrapeó, confirmó el fallo humano (conversación nueva, `"hola"` → `"Listo."`), lanzó el autor OpenCode GPT-5.6 Luna (pane `w1F:p1T` = `msg-selection-luna`), hizo 3 ciclos acotados de fix con evidencia real propia contra el sidecar pineado **1.18.25**, verificó `./scripts/verify` EXIT=0 en el worktree, y alcanzó **ROTATE_SESSION_REQUIRED_HARD (278,424 tokens)** ANTES de lanzar las reviews independientes → **Product/UX review (OpenCode DeepSeek V4 Flash) y Code/Correctness review (OpenCode Qwen 3.8 Flash) PENDIENTES (sesión FRESH siguiente).** Worktree autor limpio (working tree clean, branch `corr/msg-selection-pass` SIN mergear). Main limpio (HEAD `405ecfe` + este checkpoint doc). **M11 NO INICIADO.**
- **AUTOR (OpenCode / GPT-5.6 Luna, `msg-selection-luna`, pane `w1F:p1T`):** commits `fe636cc` (selección autoritativa: requiere mensaje assistant nuevo + turn-linked + `finish:"stop"` + solo parts de texto humano; texto vacío → NUNCA "Listo."), `ca58d19` (correlación de turno: `originating_user_id` recomputado de la lista VIVA en cada poll, no del snapshot pre-send), `969c001` (reset de grace por firma de progreso), `d0ca259` (eliminación del ack-grace early-return 15s + maquinaria muerta: espera solo `finish:"stop"` o deadline de tarea). Total 5 archivos, +298/−106 (vs base `405ecfe`).
...
- **EVALUACIÓN DE HIPÓTESIS A–E (§8):** A (modelo devolvió "Listo.") = NO para "hola" (el modelo devuelve saludo real); B (EducAI hardcodea "Listo.") = SÍ (app.rs `assistant_reply_text` fallback) — corregido; C/D (selección de mensaje anterior/intermedio / orden) = SÍ como factor secundario en turnos ≥2 por `originating_user_id` stale pre-send — corregido por `ca58d19`; E (otro mecanismo: grace 15s cortando turno lento) = SÍ — corregido por `d0ca259`.
- **RUNTIME REAL (sidecar pineado 1.18.25, `sidecars/opencode-x86_64-unknown-linux-gnu`, adapter corregido compilado, probe propio del orquestador, 3+ corridas de 4 turnos secuenciales):** TASK HOLA → saludo real ("¡Hola! ¿Cómo puedo ayudarte?…", incl. una corrida lenta de 19.3s que ANTES devolvía vacío); TASK QUESTION → "París"/"París."; TASK CREATION → texto final real ("Listo. Creé `index.html`…") + archivo `index.html` en disco (artifacts en adapter = [] porque `/diff` real devuelve `[]` para archivos commiteados → fallback documentado workspace-scan en `service.rs`, tests existentes `workspace_scan_registers_when_diff_is_empty`); TASK SEQ_Q4 → "Marte". Mensajes dumpados con IDs/finish/parentID/parts reales. Sin empty en ninguna corrida tras `d0ca259`.
...
- **`./scripts/verify` EXIT=0 EN WORKTREE** (cargo fmt/clippy/test verdes, FE 228/228, format/lint/typecheck, fetch-sidecars --check, M10 0.1.0 + UX_REDESIGN_01 contracts, cargo check src-tauri, git diff --check).
- **GATE de no-regresión (confirmado):** `finish:"tool-calls"` NO terminal; `finish:"stop"` ÚNICO terminal normal; Creation no terminal independiente; sin nudge; un turno activo por conversación; user message persistido antes de ejecución; Creation del turno origen; PREVIEW/ABRIR = PASS humano (NO reabierto); sin heurística de texto, sin filtro literal "Listo.", sin sleeps; M11 NO INICIADO. NIT menor no bloqueante: comentario duplicado en `opencode.rs:118-120` (cosmético).
- **PROXIMO PASO (sesión FRESH de orquestador):** (1) budget CONTINUE; (2) **Product/UX review FRESH** (OpenCode DeepSeek V4 Flash, rol `review`, sobre el diff `405ecfe..d0ca259`); (3) **Code/Correctness review FRESH** (OpenCode Qwen 3.8 Flash); (4) fix loop si REQUEST_CHANGES (reusar Luna `w1F:p1T` si disponible, si no Luna FRESH); (5) **merge gate §19** (fuente exacta de "Listo." conocida, "hola" responde relevante, selección identity-based/determinista, sin heurísticas, causalidad previa preservada, Creation/tool OK, turnos secuenciales OK, reviews APPROVE, runtime real PASS, Preview PASS, M11 NO INICIADO); (6) merge a main + `./scripts/verify` en main + checkpoint durable + limpieza (cerrar pane `w1F:p1T`, remover worktree tras merge); (7) **next gate: FRESH REAL APPIMAGE BUILD + TECHNICAL VERIFICATION, luego HUMAN PRODUCT-OWNER RE-ACCEPTANCE.**
