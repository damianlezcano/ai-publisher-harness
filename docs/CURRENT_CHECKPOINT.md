...
## Estado actual (CORR/STATE-LIFECYCLE PASS — AUTHOR COMPLETE, NOT HUMAN ACCEPTED, M11 NO INICIADO, 2026-09-02)

- **PASS DE CORRECCIÓN HUMANA (STATE / CREATION LIFECYCLE / PREVIEW / SHARE / CHAT UX) = AUTHOR COMPLETE. NO ES M11.** Corrige los hallazgos A–I del product owner sobre el AppImage real (falso "Listo." antes de ejecutar; Abrir en blanco; "Dejar de compartir" ilegible; actualización de Creation compartida que no llega a la URL pública; fuga de estado entre conversaciones; Enter/Shift+Enter; badge "Compartido" tarde; respuestas fuera de orden). Finding D (Cloudflare público muestra el artifact real) se PRESERVA.
- **Worktree:** `ai-publisher-corr-02-state-lifecycle`, branch `corr/state-lifecycle-pass`, base main `66d1a99`. Autor: Cursor Grok 4.6 High. El checkout de integración `ai-publisher-harness` no se tocó.
- **Clasificación de riesgo:** A, B, C, E, F, G, H, I son acotados (poller, preview routing, CSS, overwrite in-place + republish ADR-0004, frontend isolation/correlation). Sin STOP: no hubo migración de schema, no se rediseñó AgentEngine, no se reestructuró el FS. `revision` sigue en `1`.
- **A (falso Listo):** `poll_session` no trata un ack breve (`Listo.`/`OK`/…) + idle + sin artifacts como terminal; espera gracia (15s prod) y reconsulta `/diff`. Instrucción: escribir archivos primero; nunca un "Listo." suelto antes. Tests: `send_does_not_treat_brief_listo_as_complete_before_artifacts`, `brief_ack_detects_listo_and_ignores_real_replies`.
- **B (Abrir en blanco):** preview token-root (`/preview/<token>/` y sin slash) sirve `index.html`, igual que el publisher. WebView navega a `{base}index.html`. Misma Creation que Compartir. Tests: `preview_lifecycle` token root 200, `preview.rs` GET base URL, nested dirs siguen 404.
- **C (Dejar de compartir):** `.share-control-menu button.danger` usa `--danger` sobre fondo transparente (no `--danger-fg` blanco-sobre-blanco). Test: menuitem enabled + class `danger`.
- **E (update de Creation compartida):** match por kind+display_name → overwrite `outputs/<id>/` (mismo id, visibilidad, revision=1). Si el proyecto ya está published **y** la Creation registrada ya es pública, `refresh_published_snapshot` hace replace de ADR-0004 (misma `publicationRoute`). Una actividad distinta (`actividad-2`) crea id nuevo y **no** hijackea la URL. Si republish falla, el mensaje del asistente dice que el enlace no se actualizó. Tests: `later_turn_updates_the_same_web_creation_and_refreshes_publish`, `new_distinct_web_does_not_replace_an_already_published_snapshot`, `later_turn_does_not_reregister_prior_workspace_files` (M1) intacto.
- **F (fuga entre conversaciones):** `selectedIdRef` sincrónico en `openConversation`; `refreshConversation` no aplica si el id ya no es el seleccionado; `WorkspaceView` solo si `conversation.id === selectedId`; key por id; eventos `agent://task` ajenos no pintan el timeline visible. Tests: switch A/B, late foreign result, hold-open no muestra A mientras carga B.
- **G (Enter/Shift+Enter):** Enter envía; Shift+Enter newline; IME (`isComposing` / keyCode 229) no envía; whitespace no envía; busy respetado.
- **H (badge Compartido):** `onRefresh` del workspace refresca conversación **y** `project_list`. Test: badge en sidebar al compartir, sin pulsar "+".
- **I (orden de turnos):** `agent_send` persiste el mensaje de usuario **antes** de retornar; UI un turno in-flight por conversación (`sendingRef` + `onSendStart`). AgentService ya serializaba por proyecto. Tests: persist-before-run, sequential ChatPanel order, segundo send bloqueado.
- **Storage:** `docs/STORAGE_LAYOUT.md` documenta el layout real (AppImage ≠ contenedor de datos). Puntero en `docs/ARCHITECTURE.md`. Sin migración.
- **Gates:** `pnpm format:check && pnpm lint && pnpm typecheck && pnpm test` en `app/` = **227/227**. `cargo fmt --check && cargo clippy --all-targets --locked -- -D warnings && cargo test --locked` = PASS. `./scripts/verify` = EXIT=0 (incluye `cargo check` src-tauri).
...
- **STATUS: TECHNICALLY READY FOR HUMAN RE-ACCEPTANCE DE ESTE PASS. NO HUMAN ACCEPTED. M11 NO INICIADO.**
