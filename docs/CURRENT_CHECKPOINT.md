...
## Estado actual (UX POLISH PASS — IMPLEMENTACIÓN COMPLETA Y VERIFICADA EN MAIN `169f2ea`; REVIEWS INDEPENDIENTES PENDIENTES POR ROTACIÓN OBLIGATORIA DE SESIÓN ORQUESTADOR (HARD STOP ~210K); M11 NO INICIADO, 2026-09-03)

- **PASS = IMPLEMENTACIÓN COMPLETA, COMMITEADA Y VERIFICADA. NO COMPLETO: reviews independientes PENDIENTES.** El product owner re-acceptó las correcciones funcionales previas (attachment-as-input → update de Creation existente, imagen subida no crea Creation fantasma, Logs de esta sesión, Conversation Details responsivo, typed preview, modelo por-conversación, preview/Abrir) = **HUMAN-PASS**. Este pass es SOLO UX/presentación acotado (3 mejoras). **La sesión orquestador actual alcanzó ROTATE_SESSION_REQUIRED_HARD (~210K tokens acumulados, `opencode export` verificado) → `scripts/agent-launch --launch` falla cerrado (exit 11) por política → los 2 reviewers NO pudieron lanzarse desde esta sesión. Rotación obligatoria antes de lanzarlos.**
- **UX1 — Configuración separado en tabs General/Logs (completo):** `ProviderPanel.tsx` ahora separa **General** (nota de privacidad + proveedores Recomendados/Otros) y **Logs** ("Logs de esta sesión"). **General SIEMPRE es la pestaña activa al abrir** (`useState("general")` + remount de App.tsx `settingsOpen && <ProviderPanel/>`). **Logs NUNCA es default, NUNCA se recuerda entre aperturas ni tras reinicio** (estado local no persistido). Semántica ARIA tabs correcta (`role=tablist/tab/tabpanel`, `aria-selected`, `aria-controls`), roving tabindex + flechas ←/→, **foco inicial en la pestaña General** vía `initialFocusRef`. El viewer de logs conserva TODO el comportamiento aceptado: buffer in-memory ring 500 solo-proceso (`OnceLock`+`Mutex`+`VecDeque`), Limpiar, Copiar, Actualizar, `pre.session-logs` acotado 220px con scroll interno, empty state "Sin eventos todavía.", anuncio aria-live, sin persistencia. Mensajes nuevos `provider.tabs.general`/`provider.tabs.logs` (es-AR). Layout/navigation SOLO — backend de logging intacto.
- **UX2 — Listas de recursos compactas con fechas de dominio (completo):** en Conversation Details las filas pasan a 2 líneas: **primario** = nombre/título humano (materials: `displayName`; creaciones: **`displayName` = título**, ya no el kindLabel) a 0.95rem; **secundario** = metadato muted 0.8rem `humanSize · humanDate(createdAt)` (creaciones: `kindLabel · humanSize · humanDate`); **Abrir** compacto (0.8rem). **Fuentes de timestamp autoritativas de dominio (NO mtime, NO "now" inventado):** materials = `Material.created_at` (registrado con `clock.now()` al importar, expuesto como `MaterialView.createdAt` en `app.rs material_view`); creaciones = `Creation.created_at` (idem `creation_view`). Formato humano `humanDate` es-AR (`toLocaleString("es-AR", {dateStyle:"short", timeStyle:"short"})`, p.ej. "2/9/26, 3:42 p. m."), nunca timestamps crudos en UI. Una sola "Abrir carpeta contenedora" por sección preservada (2 total). Rows 61px medidos en motor real WebKit; `.dialog-body` scrollea internamente (sin scroll anidado por sección). Typed previews/modelo por-conversación/rename intactos.
- **UX3 — Delete-confirmation responsivo (completo):** fix en el primitivo compartido `.dialog` base: `max-width: min(420px, calc(100vw - 32px))` + media query `@media (max-width:520px)` (backdrop padding 8px, dialog padding 16px, `max-width: calc(100vw - 16px)`); `box-sizing: border-box` ya global; `.dialog-actions` ya `flex-wrap`. `ConfirmDialog` gana `className="confirm-dialog"`. **Sin cambios de semántica de borrado:** afirmativo "Sí" (normalización NFD + variantes), Eliminar disabled hasta válido, Cancelar nunca borra, Enter no bypasa, Escape cierra (focus trap existente), Task F intactas.
- **VERIFICACIÓN: `./scripts/verify` EXIT=0 EN MAIN POST-PASS** (cargo fmt/clippy/test verdes, FE **241/241** — antes 238 —, format/lint/typecheck, fetch-sidecars --check, M10 0.1.0 + UX_REDESIGN_01 contracts, cargo check src-tauri, git diff --check). Tests FE nuevos/actualizados: default-tab (General aria-selected + panel Logs hidden + foco en General), click explícito en Logs revela el viewer, flechas + modal no se cierra, reapertura siempre en General (sin recordar Logs), contrato logs visible/bounded tras click, compact rows con `humanDate(createdAt)` en Detalles, App.test settings al nuevo contrato (logs ocultos por default → click → visibles). `pnpm --dir app run build` OK (hashes `index-Bu6CW7uI.js` + `index-DIYuXvSr.css`).
- **REGRESIÓN VISUAL ACOTADA (harness WebKitGTK 4.1 REAL, dist REAL, mock Tauri — `/tmp/opencode/fe-render/wk_harness.py` + `report.json` + 16 PNG):** anchos 1200x760 / 800x640 / 520x640 / 420x680. **Configuración:** diálogo en viewport (680/680/488/488 px), General `aria-selected=true` + panel Logs `display:none` al abrir, tras click Logs `display:block` y `session-logs` acotado con scroll interno. **Detalles:** 15 filas @ 61px, 2 botones carpeta (uno por sección), body scrollea. **Delete:** diálogo 420/420/504/404 px (márgenes ≥8px, dentro del viewport), input a ancho completo disponible (368/368/468/368 px), box-sizing border-box, **"Sí" habilita Eliminar y Escape cierra verificados en el motor real**, acciones alcanzables, sin overflow horizontal del diálogo (el `doc_horizontal_overflow_px` >0 en 520/420 es el `body min-width:800px` preexistente de la app, NO el diálogo).
- **REVIEWS INDEPENDIENTES = PENDIENTES (NO lanzados).** Requeridos por el pass: **Product/UX = OpenCode DeepSeek V4 Flash FRESH** y **Code/A11y = OpenCode Qwen 3.8 Flash FRESH**. Worktrees de review YA CREADOS y listos (detached `169f2ea`): `../ai-publisher-ux-polish-review-ux` y `../ai-publisher-ux-polish-review-code`. Panes Herdr vacíos listos: `w1M:p2` (UX), `w1M:p3` (Code). **PRÓXIMA SESIÓN (FRESH):** verificar `bash scripts/check-session-budget` = CONTINUE (<80K); lanzar reviewers vía `scripts/agent-launch` (UX: `--role medium --provider opencode --worktree <…-ux> --pane w1M:p2 --name ux-review --launch`; Code: `--role review --provider opencode --worktree <…-code> --pane w1M:p3 --name code-review --launch`); si REQUEST_CHANGES aplicar SOLO findings acotados + re-review FRESH; luego merge gate, actualizar ESTE checkpoint, limpiar worktrees/panes, `./scripts/verify`, `check-session-budget`.
- **ESTADO DEL REPO:** main limpio (working tree clean), HEAD `169f2ea` (commit de implementación del pass UX POLISH). M11 NO INICIADO. Worktrees de review temporales creados (limpiar al cierre). AppImage previo `b961080a…` queda STALE tras este commit (rebuild solo si el humano lo pide tras aceptación del AppImage con estos cambios).
- **PENDIENTES EXPLÍCITOS (NO tocados en este pass):** (1) **USER PROMPT QUOTING / SERIALIZATION** — "hola" funciona, `"hola"` falla; SIGUE SIENDO EL **PRÓXIMO PASS FUNCIONAL**. (2) **LINUX APPIMAGE GLIBC PORTABILITY** — pendiente separado después del quoting pass.
- **STATUS: PASO COMPLETO EN IMPLEMENTACIÓN (3/3 UX) CON VERIFICACIÓN VERDE. Reviews independientes pendientes (requieren rotación del orquestador). NO HUMAN ACCEPTED para este pass.** **NO afirmar aceptación humana desde OpenCode. NO iniciar M11.**
