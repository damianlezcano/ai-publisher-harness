...
## Estado actual (CHAT TURN CAUSALITY PASS — ORQUESTADOR ROTADO ANTES DE LANZAR AUTOR, TRABAJO PRELIMINAR DE DIAGNÓSTICO REGISTRADO, M11 NO INICIADO, 2026-09-02)

- **CHAT TURN CAUSALITY / RESPONSE CORRELATION PASS = NO INICIADO (autor NO lanzado).** El orquestador (FRESH, deepseek-v4-flash) hizo bootstrap + grounding read-only y alcanzó **ROTATE_SESSION_REQUIRED (105,589 tokens)** por los reads de grounding; el gate de `scripts/agent-launch` fail-closed impidió lanzar el autor. **Ningún commit de producto en este pass.** Working tree de main limpio (HEAD `2c89ce4`). **M11 NO INICIADO.**
- **CONTEXTO HUMANO AUTORITATIVO (este pass):** PREVIEW / ABRIR = **PASS confirmado humano** (NO reabrir). Bloqueo único activo = **causalidad de turno / correlación respuesta↔turno**: el usuario manda "construí el Rosco con el adjunto" → el asistente responde "Listo." sin trabajo visible; el usuario manda "Listo por qué? dónde está?" / "dónde está?" / "me avisas?" → recién después el trabajo/Creation aparece. Patrón general: respuestas un turno atrás, respuestas que contestan al input anterior, y el usuario DEBE nudgar para que el trabajo previo aflore. **NO es un bug de copywriting de "Listo."** (prohibido ocultar/reemplazar/filtrar el texto o agregar waits arbitrarios).
- **GROUNDING READ-ONLY YA HECHO (registrado para no re-leer):** `crates/project-opencode/src/backend.rs` (loopback serve), `crates/project-opencode/src/lib.rs` (`with_directory_query`, XDG), `crates/project-agent/src/opencode.rs` (poll de `/session/status` + `/session/{id}/message?limit=1000` + `/session/{id}/diff`; watermark `before_assistant_count`; graces idle 2s / ack-sin-artefactos 15s; `is_brief_ack`; `normalize_output_path`), `crates/project-agent/src/service.rs` (`AgentService::run`, lock por proyecto, `merge_artifacts` diff→scan, registro), `crates/project-agent/src/model.rs` (sin turn_id; `AgentTask.id = "{session_id}-task"`), `crates/project-core/src/lib.rs` (Message tiene id/role/text/status/materialIds/creationIds/createdAt, SIN request_id/turn_id; append-only), `app/src-tauri/src/commands.rs` (`agent_send` = `send_message_persist` → emit `agent://task` working → thread detached `send_message_run` → emit completed/failed/cancelled; `AgentTaskEvent {project_id,status,message,registered_creation_ids}`), `crates/project-app/src/app.rs` (`send_message`/`send_message_persist`/`send_message_run`/`run_agent_with_inputs`; append_assistant_message con creation_ids; text de error), `app/src/App.tsx` (listener `agent://task`, `inFlightRef`, `selectedIdRef`, `refreshConversation` si id sigue seleccionado), `app/src/components/WorkspaceView.tsx` (`sendingRef`, `pendingUser` optimista, `onSendStart`/`onSendEnd`, `api.agentSend`), `app/src/components/ChatPanel.tsx` (timeline por `createdAt`, bubble assistant, cards Creation vía `creationIds`), `crates/fake-opencode-server/src/lib.rs` (server de test: `status_sequence`, `prompt_appends_response` append synthetic assistant, `set_diff_body`, etc.).
- **HIPÓTESIS PRELIMINAR (a confirmar/refutar por el autor con evidencia runtime, NO tomada como verdad):** la correlación hoy es por **watermark de conteo de mensajes assistant** (`before_assistant_count`) + **"último texto assistant no vacío"** + fase idle/complete del `/session/status`, NO por identidad lógica de turno ni por el mensaje user específico. `send_message_run` apendea la respuesta al timeline por conversación (no por turn_id). No existe request_id/turn_id en el modelo de datos. Zonas a investigar por el autor (del brief misión §6): ordenamiento de eventos de sesión/mensajes OpenCode; si "primer texto assistant no vacío" se trata como terminal; si OpenCode queda idle entre planning y tool execution; si la UI/poller para muy temprano; si el próximo user message drena eventos viejos pendientes; correlación solo por conversación; timing del scan de Creation (¿depende de un segundo mensaje o poll posterior?). REQUIRED INVARIANT: cada envío tiene turn_id/request_id con lifecycle QUEUED→RUNNING→WORKING→COMPLETED (o FAILED/CANCELLED); texto transitorio ("Listo."/"Voy a hacerlo."/"Déjame revisar...") NO cierra el turno; un turno solo es completo con señal terminal REAL de la ejecución del agente para ESE turno (idle/sesión tras todo el trabajo, evento final, ausencia de trabajo pendiente — verificar comportamiento real de opencode 1.18.25, no inventar semántica). UN TURNO AGENTE ACTIVO POR CONVERSACIÓN (serialización real end-to-end, no solo UI). Creation coordinada con el lifecycle del turno (sin depender del nudge/segundo mensaje). Preservar: preview/ABRIR, adjuntos, cards, publicación Cloudflare real, modelo en Configuración, delete «Sí», aislamiento de conversaciones, Enter/Shift+Enter, badge compartido, layout/storage. Matriz de tests CASE A–H y traza diagnóstica T0–T8 con secuencia de eventos BEFORE/AFTER (no screenshots).
- **PROXIMO PASO (sesión FRESH de orquestador):** (1) verificar budget CONTINUE; (2) lanzar autor FRESH Cursor/Grok 4.6 High (`scripts/agent-launch --role high-visual --provider cursor --launch` en pane Herdr sibling con cwd = worktree; crear worktree `corr/chat-causality-pass` en `../ai-publisher-corr-03-chat-causality`; NUNCA lanzar con cwd = main checkout — el agente debe vivir en su worktree); (3) seguir la secuencia misión §13–§22 (autor → inspección orquestador → Product/UX Cursor Grok FRESH → Code/Correctness opencode-go/qwen3.8-flash FRESH → fix loop → merge gate → `./scripts/verify` en main); (4) checkpoint durable + limpieza.
- **GATES de no-regresión (confirmados en estado previo):** `./scripts/verify` EXIT=0 en main `2c89ce4` (FE 228/228, cargo verde, contracts M10 + UX_REDESIGN_01, fetch-sidecars --check). AppImage fresco `40403c69…` construido y verificado técnicamente. **PREVIEW/ABRIR = PASS humano. NO HUMAN ACCEPTED.**
