...
This document is the Herdr/worktree delegation procedure and independent-review
mechanics. Cost/session policy is `docs/engineering/AGENT_POLICY.md`; role
execution contracts are `prompts/`.
...
pool using `docs/engineering/AGENT_POLICY.md`; no provider, model, or Antigravity is
...
2. Lead assigns a separate worktree/diff review to a different pool,
   with `docs/product/SECURITY.md` invariants 1–7 and 9–11 as explicit review criteria.
