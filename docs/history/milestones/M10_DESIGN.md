...
- `docs/history/milestones/README.md` — `M10 Packaging`.
...
- `docs/history/milestones/M9_DESIGN.md` §2/§36 — `M10 = Packaging. Linux AppImage then RPM;
  bundle/manage OpenCode and cloudflared as pinned sidecars; native CI for future
  Windows.`
- `docs/distribution/PLATFORM_POLICY.md` — target Linux distribution artifacts in this order:
...
docs/engineering/VERIFY.md                         UPDATE M10 section + gate
...
| 4 | verify gate + docs (VERIFY.md M10, version check) | MEDIUM | 1, 3 | `m10/verify` | scripts/verify, docs/engineering/VERIFY.md |
...
+ `docs/engineering/VERIFY.md`. `scripts/verify` is touched only by T4 (and T6's gate).
...
`docs/engineering/VERIFY.md` gains a short M10 section documenting that M10 is packaging +
