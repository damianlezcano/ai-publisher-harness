...
2. This contract.
3. `docs/engineering/REQUIREMENTS.md` and the selected requirement after it is located.
...
5. **Route** each ready task with `scripts/agent-class-resolve` (see Routing).
   Default Worker tier is `cheap`. Escalate only as `docs/engineering/AGENT_POLICY.md`
...
escalation reason per `docs/engineering/AGENT_POLICY.md`.

## Delegation

Mechanism: `docs/engineering/MULTI_AGENT_WORKFLOW.md` + `docs/engineering/WORKTREES.md` +
...
delegated workers** in `docs/engineering/MULTI_AGENT_WORKFLOW.md`.
